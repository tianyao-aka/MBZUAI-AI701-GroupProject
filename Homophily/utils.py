...
def load_data(dset_name,walk_length,walk_num):
    node_struct_features = []
    dset = Planetoid(root='dataset/', name=dset_name,split='full')
    # print(dset[0])
...
def save_node_img(index,dataset_name='Cora'):
    node_imgs, node_imgs_feature, node_struct_features, isolated_node_tag, dset, k_hop_neibrs = load_data(
        dataset_name, 10, 10)
    s = {'img': node_imgs, 'strcuture:': node_struct_features}
    if not os.path.exists(f'data/{dataset_name}_processed/'):
        os.makedirs(f'data/{dataset_name}_processed/',exist_ok=True)
    torch.save(s, f'data/{dataset_name}_processed/data_point_{index}.pt')
    print ('done saving node image, current index:',index)
    index +=1
...
if __name__=='__main__':
    ...
    save_node_img(index,dataset_name=dset_name)
