...
class PL_Model(pl.LightningModule):
    def __init__(self, in_dim,hidden_dim,struct_dim,num_classes,walk_len = 10,num_walk=10,lr=1e-3,weight_decay=1e-6,use_gpu=False,layer_num=3,path=None):
        super().__init__()
        ...
        self.fpath = path
        self.data_list = glob(f'{path}/*.pt')
        # init data


        if 'Cora' in path:
            _, _, _, _, dset, k_hop_neibrs = load_data('Cora', walk_len, num_walk)
            dset = Planetoid(root='dataset/', name='Cora',split='full')[0]
        elif 'Citeseer' in path:
            _, _, _, _, dset, k_hop_neibrs = load_data('CiteSeer', walk_len, num_walk)
            dset = Planetoid(root='dataset/', name='CiteSeer', split='full')[0]
        elif 'PubMed' in path:
            _, _, _, _, dset, k_hop_neibrs = load_data('PubMed', walk_len, num_walk)
            dset = Planetoid(root='dataset/', name='PubMed', split='full')[0]
        ...
        self.val_mask = dset.val_mask.to(self.dev)
        self.test_mask = dset.test_mask.to(self.dev)
        self.train_mask = torch.logical_not(torch.logical_or(self.val_mask, self.test_mask))  # fully supervised
        # self.train_mask = dset.train_mask
...
if __name__=='__main__':
    ...
    parser.add_argument('--layer_num', type=int, default=3,
                        help='model layer')
    parser.add_argument('--dataset_path', type=str, default='data/Cora_processed/',
                        help='dataset path')
    ...
    for _ in range(4):
        # 4 trials
        model = PL_Model(num_f, 64, 6, 7, use_gpu=True, walk_len=10, num_walk=10, layer_num=layer_num,path=path)
    ...
    print ('mean acc and max acc are:',acc.mean(),acc.max())
